/*
 * zm.c - ZeroMount CLI Userspace Tool
 */
...
#define IOCTL_ADD       0x40185A01
#define IOCTL_DEL       0x40185A02
#define IOCTL_CLEAR     0x5A03
#define IOCTL_VER       0x80045A04
#define IOCTL_ADD_UID   0x40045A05
#define IOCTL_DEL_UID   0x40045A06
#define IOCTL_LIST      0x80045A07
#define IOCTL_ENABLE    0x5A08
#define IOCTL_DISABLE   0x5A09

#define ZM_ACTIVE 1
#define ZM_DIR    128
#define PATH_MAX  4096

#define WERR(s) sys3(SYS_WRITE, 2, (long)(s), sizeof(s)-1)
...
void c_main(long *sp) {
    ...
    if (argc < 2) {
        WERR("Usage: zm add|del|clear|blk|unb|list|ver|enable|disable\n");
        goto do_exit;
    }

    fd = sys4(SYS_OPENAT, AT_FDCWD, (long)"/dev/zeromount", O_RDWR, 0);
    if (fd < 0) {
        WERR("zm: cannot open /dev/zeromount\n");
        exit_code = 2;
        goto do_exit;
    }
    ...
    if (cmd == 'a') needed = 4;
    else if (cmd == 'e' || (cmd == 'd' && cmd2 == 'i')) needed = 2;
    else if (cmd != 'c' && cmd != 'v' && cmd != 'l') needed = 3;

    if (argc < needed) {
        WERR("zm: missing arguments\n");
        goto close_and_exit;
    }
    ...
    if (cmd == 'e') {
    ...
    else if (cmd == 'd' && cmd2 == 'i') {
        ioctl_code = IOCTL_DISABLE;
    }
    else if (cmd == 'a' || (cmd == 'd' && cmd2 == 'e')) {
        #if defined(__aarch64__)
        ...
        if (cmd == 'd') {
            ...
            if (sys4(SYS_FSTATAT, AT_FDCWD, (long)dst_ptr, (long)stat_buf, 0) == 0) {
                ...
                if ((mode & 0170000) == 0040000) data.flags |= ZM_DIR;
            }
        ...
        while (*s) {
            ...
            unsigned int new_uid = uid * 10 + (c - '0');
            if (new_uid < uid) {
                WERR("zm: UID overflow\n");
                goto close_and_exit;
            }
            uid = new_uid;
        }
    ...
    if (ioctl_code) {
        ...
        if (cmd == 'v' && res > 0) {
            ...
            v_buf[pos++] = '\n';
            sys3(SYS_WRITE, 1, (long)v_buf, pos);
        }
        else if (cmd == 'l' && res > 0) {
