...
# Source unified logging if available, otherwise stub out log functions
if [ -f "$MODDIR/logging.sh" ]; then
    . "$MODDIR/logging.sh"
    log_init "monitor"
else
    log_info() { :; }; log_err() { :; }; log_warn() { :; }; log_debug() { :; }
fi

find_zm_binary() {
    case "$(uname -m)" in
        aarch64) _abi=arm64-v8a; _arch=arm64 ;;
        armv7*|armv8l) _abi=armeabi-v7a; _arch=arm ;;
        x86_64) _abi=x86_64; _arch=x86_64 ;;
        i686|i386) _abi=x86; _arch=x86 ;;
    esac
    for path in "$MODDIR/bin/${_abi}/zeromount" "$MODDIR/zm-${_arch}" "$MODDIR/bin/zm" "$MODDIR/zm"; do
...
generate_status_cache() {
    ...
    local rules_count=$("$LOADER" vfs list 2>/dev/null | grep -cv '^no rules$' || echo 0)
    local excluded_count=$(wc -l < "$ZEROMOUNT_DATA/.exclusion_list" 2>/dev/null || echo 0)
    local hidden_count=$(count_hidden_paths)
    local driver_ver=$("$LOADER" status --json 2>/dev/null | grep -o '"driver_version":[0-9]*' | grep -o '[0-9]*' || echo "1")
    ...
    local loaded_modules=$("$LOADER" vfs list 2>/dev/null | grep -v '^no rules$' | awk -F'->' '{print $1}' | grep -oE '/data/adb/modules/[^/]+' | sort -u | while read p; do basename "$p"; done | tr '\n' ',' | sed 's/,$//')
...
register_module() {
    ...
    for partition in $TARGET_PARTITIONS; do
        ...
        find "$mod_path/$partition" -type f 2>/dev/null | while read -r rel; do
            ...
            local rpath="$rel"
            "$LOADER" vfs add "$vpath" "$rpath" </dev/null 2>/dev/null
...
unregister_module() {
    ...
    while IFS='|' read -r vpath _type _extra; do
        [ -z "$vpath" ] && continue
        "$LOADER" vfs del "$vpath" </dev/null 2>/dev/null && count=$((count + 1))
    done < "$tracking_file"
...
sync_module() {
    ...
    while IFS='|' read -r vpath _type; do
        [ -z "$vpath" ] && continue
        grep -qxF "$vpath" "$tmp" || { "$LOADER" vfs del "$vpath" </dev/null 2>/dev/null; removed=$((removed + 1)); }
    done < "$tracking_file"

    while IFS= read -r vpath; do
        [ -z "$vpath" ] && continue
        grep -q "^${vpath}|" "$tracking_file" || {
            "$LOADER" vfs add "$vpath" "$mod_path$vpath" </dev/null 2>/dev/null
            added=$((added + 1))
        }
    done < "$tmp"
