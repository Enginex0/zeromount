...
import { runShell, ksuWriteFile } from '../../lib/ksuApi';
...
export function AboutSection() {
  const copyDebugInfo = async () => {
    try {
      const { errno, stdout } = await runShell(`${PATHS.BINARY} log dump`);
      if (errno === 0) {
        const parsed = JSON.parse(stdout);
        store.showToast(t('toast.logsSaved', { path: parsed.zip }), 'success');
      } else {
        store.showToast(t('toast.failedDumpLogs'), 'error');
      }
    } catch {
      store.showToast(t('toast.failedDumpLogs'), 'error');
    }
  ...
  const exportConfig = async () => {
    try {
      const { errno, stdout } = await runShell('cat /data/adb/zeromount/config.toml');
      if (errno !== 0 || !stdout) {
        store.showToast(t('toast.failedReadConfig'), 'error');
        return;
      }
      ...
      if (writeErr === 0) {
        store.showToast(t('toast.configExported'), 'success');
      } else {
        store.showToast(t('toast.failedExportConfig'), 'error');
      }
    } catch {
      store.showToast(t('toast.failedExportConfig'), 'error');
    }
  ...
  const importConfig = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (!content.trim()) {
          store.showToast(t('toast.emptyFile'), 'error');
          return;
        }
        ...
        if (errno !== 0) {
          store.showToast(t('toast.failedWriteConfig'), 'error');
          return;
        }
        ...
        store.showToast(t('toast.failedImportConfig'), 'error');
      }
    };
    input.click();
  };
  ...
  return (
    <Card>
      ...
      <div class="settings__about">
        ...
        <div class="settings__about-title">{t('about.appName')}</div>

        <div class="settings__about-badge">{t('about.badge')}</div>

        <button
          class="settings__repo-btn"
          onClick={() => runShell(`am start -a android.intent.action.VIEW -d '${GITHUB_URL}'`)}
        >
        ...
        <div class="settings__actions">
          ...
          <Button variant="secondary" size="small" onClick={importConfig}>
            {t('about.importConfig')}
          </Button>
        </div>

        <div class="settings__footer">
          {t('about.footerMadeWith')} <span class="gradient-text">{t('about.footerPassion')}</span> {t('about.footerForThe')} <span class="gradient-text">{t('about.footerCommunity')}</span>
        </div>
      </div>
    </Card>
  );
